import unittest

from selenium import webdriver
from selenium.webdriver.chrome.service import Service
from selenium.webdriver.common.action_chains import ActionChains
from selenium.webdriver.common.by import By
from webdriver_manager.chrome import ChromeDriverManager


class TestSignIn(unittest.TestCase):
    ACCOUNT_BUTTON = (By.XPATH, '//*[@id="nav-link-accountList-nav-line-1"]')
    SIGN_IN_BUTTON = (By.XPATH, '//*[@id="nav-flyout-ya-signin"]/a/span')
    INPUT_MAIL_BOX = (By.XPATH, '//*[@id="ap_email"]')
    CONTINUE_BUTTON = (By.XPATH, '//*[@id="continue"]')
    ERROR_MESSAGE = (By.XPATH, '//*[@id="auth-error-message-box"]/div/h4')
    chrome = None

    def setUp(self):
        s = Service(ChromeDriverManager().install())
        self.chrome = webdriver.Chrome(service=s)
        self.chrome.maximize_window()
        self.chrome.get('https://www.amazon.com/')

    def tearDown(self):
        self.chrome.quit()

    def test_sign_in_access_page(self) -> None:
        a = ActionChains(self.chrome)
        account_button = self.chrome.find_element(*self.ACCOUNT_BUTTON)
        a.move_to_element(account_button).perform()
        sign_in_button = self.chrome.find_element(*self.SIGN_IN_BUTTON)
        a.move_to_element(sign_in_button).click().perform()

        self.chrome.find_element(*self.INPUT_MAIL_BOX).send_keys('enter_valid_mail@gmail.com')
        self.chrome.find_element(*self.CONTINUE_BUTTON).click()

        elem = self.chrome.find_element(*self.ERROR_MESSAGE)
        self.assertTrue(elem.is_displayed(), 'Mesajul nu e vizibil')



